...
import { ContextualAffiliateCard, GuideLink, TripChecklist } from "@/components/TripGuideActions";
...
export function TripRealityGuidePage({ guide }: { guide: TripRealityGuide }) {
  ...
  const ports = liveData.cruisePorts.filter((port) => guide.liveAirportIds.includes(port.portId));
  const liveIsCurrent = liveData.status === "current";
  const jsonLd = {
    "@context": "https://schema.org",
    "@graph": [
      { "@type": "Article", headline: guide.title, description: guide.description, mainEntityOfPage: `https://cruisedealsflorida.org${route}`, image: guide.heroImage, author: { "@type": "Organization", name: "Florida Deals Network" }, publisher: { "@type": "Organization", name: "Cruise Deals Florida" } },
      { "@type": "BreadcrumbList", itemListElement: [
        { "@type": "ListItem", position: 1, name: "Cruise Deals Florida", item: "https://cruisedealsflorida.org" },
        { "@type": "ListItem", position: 2, name: "Trip Reality Guides", item: "https://cruisedealsflorida.org/trip-reality" },
        { "@type": "ListItem", position: 3, name: guide.title, item: `https://cruisedealsflorida.org${route}` }
      ] }
    ]
  };
    ...
    <>
      ...
      <main className="bg-[#f8fafc]">
        <section className="mx-auto max-w-7xl px-4 pb-10 pt-8 sm:px-6 lg:px-8">
          <nav className="text-sm font-bold text-slateText" aria-label="Breadcrumb"><Link href="/">Cruises</Link><span className="px-2">/</span><span>Trip Reality Guide</span></nav>
          <div className="mt-6 grid overflow-hidden rounded-lg border border-slate-200 bg-white shadow-card lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="relative min-h-72 lg:min-h-full"><FallbackImage alt={guide.heroAlt} className="object-cover" fill priority sizes="(max-width: 1024px) 100vw, 45vw" src={guide.heroImage} /></div>
          </div>
        </section>
        ...
        {guide.affiliate === "esimshop" ? <section className="mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8"><ContextualAffiliateCard articleCluster="florida_cruise_preparation" ctaText="Explore international eSIM plans" href={ESIMSHOP_URL} placement={guide.affiliatePlacement ?? "travel_preparation"} route={route} travelerIntent="foreign_port_connectivity" /></section> : null}
        ...
        <TripRetentionTools image={guide.heroImage} route={route} site="cruisedealsflorida" title={guide.title} />
      </main>
      <SiteFooter />
    </>
  );
