...
export function TripChecklist({ route, storageKey, items }: { route: string; storageKey: string; items: string[] }) {
  ...
  async function copyChecklist() {
    ...
    emit("trip_checklist_interaction", { network: "Florida Deals Network", site: "cruisedealsflorida.org", route, action: "copy", completed_items: Object.values(checked).filter(Boolean).length });
  }
  ...
  return (
    <section className="rounded-lg border border-sky-200 bg-sky-50 p-5 sm:p-7" aria-labelledby={`${storageKey}-title`}>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <button className="btn btn-secondary min-h-11 px-4" onClick={() => { window.print(); emit("trip_checklist_interaction", { network: "Florida Deals Network", site: "cruisedealsflorida.org", route, action: "print", completed_items: Object.values(checked).filter(Boolean).length }); }} type="button"><Printer className="h-4 w-4" /> Print</button>
...
export function ContextualAffiliateCard({
  ...
  ctaText: string;
  href: string;
  placement: "cruise_wifi_comparison" | "foreign_port_connectivity" | "travel_preparation";
  route: string;
  ...
  return (
    <aside className="rounded-lg border border-sky-200 bg-white p-5 shadow-card sm:p-6" aria-label="Affiliate partner option">
      ...
      <p className="mt-3 text-sm font-semibold leading-6 text-slateText">Useful for an unlocked, compatible phone when a traveler needs cellular data in supported foreign ports or countries. It does not provide ship Wi-Fi or guarantee service at sea.</p>
